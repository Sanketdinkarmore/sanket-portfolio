...
import { FolderGit2, LayoutGrid, ExternalLink, Github } from 'lucide-react';
...
const PortfolioCard = ({
  ...
  return (
    ...
    >
      <div className="border border-white/10 rounded-2xl p-2">
        <div
          className="relative rounded-2xl overflow-hidden cursor-pointer w-full bg-[#1a1c21]"
        ...
        >
          <img
            src={image}
            alt={title}
            className="block w-full h-auto max-h-[280px] object-cover object-top transition-all duration-500"
            style={{
              transform: hovered ? 'scale(1.05)' : 'scale(1)',
              opacity: hovered ? 0.12 : 0.85,
            }}
          />

          <div className="absolute top-4 left-4 z-10">
            <span className="text-sm font-semibold px-4 py-1.5 rounded-full shadow-lg bg-orange-500 text-white">
              {category}
            </span>
          </div>
        ...
        <div className="mt-4 px-1">
          <h3 className="text-white text-xl font-extrabold mb-3 leading-snug">
            {title}
          </h3>
          <div className="flex flex-wrap gap-2">
            ...
            {githubUrl && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 active:scale-95 transition-all duration-200 text-white text-sm font-semibold px-5 py-2.5 rounded-lg"
              >
                Github
                <Github size={13} />
              </a>
...
const Portfolio = () => {
  ...
  return (
    <div className="min-h-screen text-white px-2 py-12 sm:px-8 md:px-5 lg:px-7">
      ...
      <motion.div layout className="columns-1 sm:columns-2 gap-6 max-w-full">
